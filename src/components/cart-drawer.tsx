          ...
          >
            <div className="flex items-center justify-between border-b border-black/10 pb-5">
              <div>
                ...
                <h2 className="font-serif text-3xl">Carrello</h2>
              </div>
              <button aria-label="Chiudi carrello" onClick={() => setIsOpen(false)}>
                <X />
              </button>
            </div>
                ...
                <div className="grid h-full place-items-center text-center">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-black/55">
                      Scegli qualcosa di buono dalla Sicilia.
                    </p>
                  </div>
                </div>
              ) : (
                items.map((item) => (
                  <div className="flex gap-4 border-b border-black/10 pb-5" key={item.slug}>
                    ...
                    <div className="flex min-w-0 flex-1 flex-col justify-between">
                      <div>
                        <p className="font-serif text-lg leading-tight">{item.shortName}</p>
                        <p className="mt-1 text-xs uppercase tracking-widest text-black/45">
                          {item.volume} · Qtà {item.quantity}
                        </p>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">{formatPrice(item.price * item.quantity)}</span>
                        <button
                          aria-label={`Rimuovi ${item.shortName}`}
            ...
            {items.length > 0 && (
              <div className="border-t border-black/10 pt-5">
                ...
                <button className="button-primary w-full">Procedi al checkout</button>
                <button
                ...
                <p className="mt-4 text-center text-[11px] leading-relaxed text-black/45">
                  Demo front-end: il pagamento verrà collegato al provider e-commerce scelto.
                </p>
              </div>
            )}
            <Link className="sr-only" href="/shop">Vai allo shop</Link>
          </aside>
        </div>,
        document.body,
      )}
